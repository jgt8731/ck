...
<%
	String keyField = "", keyWord = ""; // 검색기준값과 검색을 전달받아. 저장할 변수
	...
	*/
	...
	int totalRecord = (v == null) ? 0 : v.size();	// 게시판에 저장된 전체 글의 갯수 저장
	int numPerPage = 25;		// 한 페이지당 보여질 글의 개수 저장
	int pagePerBlock = 3;		// 한 블럭당 묶여질 페이지 개수 저장
...
<tr>
	<td align=center colspan=2>
		...
		%>
					<a href="#" onclick="fnRead('<%=num%>'); return false;"><%= dto.getSubject() %></a>
				</td>
...
<tr>
	...
	<%
		if(nowBlock > 0) {
	%>
			<a class="prev" href="List.jsp?nowBlock=<%= nowBlock - 1 %>&nowPage=<%= ((nowBlock - 1) * pagePerBlock) %>">
			◀◀◀
			</a>
	<%
		}
	...
	<%
		for(int i = 0; i < pagePerBlock; i++) {
			int nowPageTmp = (nowBlock * pagePerBlock) + i + 1;
			if(nowPageTmp > totalPage) break;
	%>
			<a href="List.jsp?nowBlock=<%= nowBlock %>&nowPage=<%= nowPageTmp - 1 %>">
			<%= nowPageTmp %>
			</a>
	<%
		}
	...
	%>
			<a class="next" href="List.jsp?nowBlock=<%= nowBlock + 1 %>&nowPage=<%= ((nowBlock + 1) * pagePerBlock) %>">
			▶▶▶
			</a>
	<%
		}
	...
	<tr>
		<td align=center valign=bottom>
			<select name="keyField" size="1">
				<option value="name" <% if("name".equals(keyField)) { %> selected <% } %>> 이름
				<option value="subject" <% if("subject".equals(keyField)) { %> selected <% } %>> 제목
				<option value="content" <% if("content".equals(keyField)) { %> selected <% } %>> 내용
			</select>
